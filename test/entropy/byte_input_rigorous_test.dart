...
import 'package:j2k/src/j2k/entropy/decoder/byte_input_buffer.dart';
import 'package:j2k/src/j2k/entropy/decoder/byte_to_bit_input.dart';
...
void main() {
  ByteToBitInput buildInput(Uint8List data, {int offset = 0, int? length}) {
  ...
  List<int> bitsOf(int value, {int width = 8}) {
  ...
  List<int> readBits(ByteToBitInput input, int count) {
    return List<int>.generate(count, (_) => input.readBit());
  }

  int readAlignedByte(ByteToBitInput input) {
    var value = 0;
    for (var i = 0; i < 8; i++) {
  ...
  List<int> readAlignedBytes(ByteToBitInput input, int count) {
    return List<int>.generate(count, (_) => readAlignedByte(input));
  }

  group('ByteToBitInput - Operações Básicas RIGOROSAS', () {
    test('readBit - cada bit de 0xAC (10101100)', () {
      final data = Uint8List.fromList([0xAC]);
      final input = buildInput(data);
    ...
    test('readBit reconstrói bytes sem stuffing', () {
      ...
      expect(readAlignedBytes(input, data.length), data);
    });
  });
  ...
  group('ByteToBitInput - Byte Stuffing CRÍTICO', () {
    test('0xFF 0x00 consome apenas 7 bits do byte seguinte', () {
      ...
      expect(readBits(input, 8), everyElement(equals(1)));
      expect(readBits(input, 7), everyElement(equals(0)));

      final proximo = <int>[input.readBit(), ...readBits(input, 7)];
      expect(proximo, equals(bitsOf(0xAB)));
    });
    ...
    test('0xFF 0x00 antes de 0x12 mantém a sequência correta', () {
      ...
      expect(readBits(input, 8), everyElement(equals(1)));
      expect(readBits(input, 7), everyElement(equals(0)));
      expect(readBits(input, 8), equals(bitsOf(0x12)));
    });

    test('Múltiplos 0xFF 0x00 consecutivos mantêm apenas 7 bits úteis', () {
      final data =
          Uint8List.fromList([0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0x99]);
      final input = buildInput(data);

      for (var i = 0; i < 3; i++) {
        expect(readBits(input, 8), everyElement(equals(1)),
            reason: '0xFF número $i');
        expect(readBits(input, 7), everyElement(equals(0)),
            reason: 'stuffing $i');
      }

      final restante = <int>[input.readBit(), ...readBits(input, 7)];
      expect(restante, equals(bitsOf(0x99)));
    });
    ...
    test('0xFF 0x01 ainda descarta o bit forçado a zero', () {
      ...
      expect(readBits(input, 8), everyElement(equals(1)));
      expect(readBits(input, 7), equals([0, 0, 0, 0, 0, 0, 1]));
    });
  });
  ...
  group('ByteToBitInput - Offset e Length RIGOROSOS', () {
    test('Offset correto', () {
      ...
      final input =
          buildInput(data, offset: 2, length: 2); // Offset 2, length 2

      expect(readAlignedBytes(input, 2), equals([0x22, 0x33]));
    });

    test('Length exato', () {
      final data = Uint8List.fromList([0xAA, 0xBB, 0xCC]);
      final input = buildInput(data, length: 2); // Apenas 2 bytes

      expect(readAlignedBytes(input, 2), equals([0xAA, 0xBB]));
  ...
  group('ByteToBitInput - Casos que Podem Causar Bug no MQDecoder', () {
    test('Padrão real de bitstream JPEG2000', () {
      ...
      final data =
          Uint8List.fromList([0x84, 0x00, 0xFF, 0x00, 0x20, 0xFF, 0x00, 0x10]);
      final input = buildInput(data);

      expect(readBits(input, 8), equals(bitsOf(0x84)));
      expect(readBits(input, 8), equals(bitsOf(0x00)));
      expect(readBits(input, 8), everyElement(equals(1)));
      expect(readBits(input, 7), everyElement(equals(0)));
      expect(readBits(input, 8), equals(bitsOf(0x20)));
      expect(readBits(input, 8), everyElement(equals(1)));
      expect(readBits(input, 7), everyElement(equals(0)));
      expect(readBits(input, 8), equals(bitsOf(0x10)));
    });
    ...
    test('Leitura de bits com byte stuffing no meio', () {
      ...
      expect(readBits(input, 7), everyElement(equals(0)));
      expect(readBits(input, 8), equals(bitsOf(0x0F)));
    });

    test('Overflow de buffer - não deve crashar', () {
      final data = Uint8List.fromList([0x12]);
      final input = buildInput(data, length: 1);

      expect(readAlignedBytes(input, 1), equals([0x12]));
  ...
  group('ByteToBitInput - Comparação com Comportamento Java', () {
    test('Sequência idêntica ao TestDecoder.java', () {
      ...
      final data =
          Uint8List.fromList([0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
      final input = buildInput(data);

      expect(readAlignedBytes(input, data.length), equals(data));
    });
